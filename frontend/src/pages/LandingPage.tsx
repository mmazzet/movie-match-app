...
function LandingPage() {
  ...
  return (
    <div className="relative w-full min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-900 overflow-hidden">
      ...
      <div className="absolute inset-0 bg-radial-red opacity-20 pointer-events-none" style={{ background: 'radial-gradient(circle at 20% 50%, rgba(239, 68, 68, 0.1), transparent 50%)' }}></div>
      <div className="absolute inset-0 bg-radial-blue opacity-20 pointer-events-none" style={{ background: 'radial-gradient(circle at 80% 80%, rgba(59, 130, 246, 0.05), transparent 50%)' }}></div>

      {/* Navbar */}
      <nav className="relative z-10 flex justify-between items-center px-6 md:px-8 py-4 md:py-6">
        <div className="text-xl md:text-2xl font-black text-white tracking-tight">🎬 MovieMatch</div>
      ...
      <div className="relative z-5 flex flex-col justify-center items-center min-h-[calc(100vh-100px)] px-4 md:px-8 py-8 text-center">
        <h1 className="text-4xl md:text-6xl lg:text-7xl font-black text-white mb-6 md:mb-8 tracking-tight leading-tight max-w-4xl">
          ...
          <br />
          <span className="bg-gradient-to-r from-red-500 to-pink-500 bg-clip-text text-transparent">together</span>
        </h1>
