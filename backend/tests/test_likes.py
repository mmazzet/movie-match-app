...
def test_like_same_movie_twice(client):
    ...
    # Like once
    client.post("/api/v1/likes", json=movie, headers={"Authorization": f"Bearer {token}"})

    # Like again
    response = client.post("/api/v1/likes", json=movie, headers={"Authorization": f"Bearer {token}"})
...
def test_unlike_a_movie(client):
    ...
    # Unlike it
    response = client.delete("/api/v1/likes/693134", headers={"Authorization": f"Bearer {token}"})
